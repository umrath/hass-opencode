...
class TestDesktopService(unittest.TestCase):
    ...
    def test_single_instance_binds_all_interfaces(self):
        """W1: In non-proxy mode (PORT=8099), ttyd must bind 0.0.0.0.
        In proxy mode (PORT=8098), it must bind 127.0.0.1.
        Verify the BRANCH assignments, not just the exec line."""
        # Non-proxy branch: PORT=8099, IFACE_ARG must be empty
        self.assertIn('PORT=8099\n    IFACE_ARG=""', self.text,
            "non-proxy branch must set IFACE_ARG empty (bind 0.0.0.0)")
        # Proxy branch: PORT=8098, IFACE_ARG must be -i 127.0.0.1
        self.assertIn('IFACE_ARG="-i 127.0.0.1"', self.text,
            "proxy branch must set IFACE_ARG for loopback binding")
